            ...
            <div class="card">
                <div class="card-body">
                  <h4 class="header-title"> List</h4> 
                    ...
                    <form method="POST" action="{{ URL::route('article-html.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row mt-5">
                                    ...
                                    <select class="form-select" id="title" name="title_id">
                                      <option selected disabled value="">Open this select menu</option>
                                        @foreach ($articles as $row)
                                            @php
                                                // Check if this article ID exists in articleHtmls
                                                $isDisabled = $articleHtmls->contains('title_id', $row->id);
                                            @endphp
                                            <option value="{{ $row->id }}" @if ($isDisabled) disabled class="text-danger" @endif>{{ '(' . $row->id . ')' . Str::limit($row->title, 50, '...') }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-lg-12">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="blog_articles" name="blog_article" rows="8"></textarea>
                                </div>
                            </div>
