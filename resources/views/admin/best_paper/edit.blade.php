    ...
    <div class="row mb-3 pt-2" style="border:1px solid #000;">
        <div class="col-12">
            <!-- Flash Message Display -->
                ...
                @endif
        <form action="{{ route('award.update', $bestpapers->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        <div class="row">
          <div class="col-12">
              ...
              <select class="form-control" name="journal_id" id="category" required>
                    ...
                    @foreach($journals as $journal)
                        <option value="{{ $journal->id }}" 
                                {{ $journal->id == $bestpapers->journal_id ? 'selected' : '' }}>
                            {{ $journal->title }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
          <div class="col-12">
            <div class="form-group">
              ...
              <textarea class="form-control summernote" id="thirdContent" name="bestpaper"  rows="8" placeholder="Enter Aims & Scope" require>{{$bestpapers->bestpaper}}</textarea>
            </div>
          </div>
        </div>
